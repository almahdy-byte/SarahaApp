...
export const register = async (req, res ,next) => {
    ...
    if (isEXisted)
         return next(new Error("Email is already existed"), { cause: StatusCodes.BAD_REQUEST });
...
export const login = async (req, res ,next) => {
    ...
    if(!user)
         return next(new Error("Email or password is incorrect"), { cause: StatusCodes.BAD_REQUEST });
    ...
    if(user.confirmed === false) 
        next(new Error("In-valid Code Email is not confirmed"), { cause: StatusCodes.BAD_REQUEST });
...
export const confirmEmail = async (req, res , next) => {
    ...
    if(!user) 
        return next(new Error("Email is not existed"), { cause: StatusCodes.BAD_REQUEST });

    if(!compare(code , user.confirmEmailOTP))
         return next(new Error("code is not correct"), { cause: StatusCodes.BAD_REQUEST });
